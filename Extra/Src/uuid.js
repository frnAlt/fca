...
try {
  ...
  base32 = {
    encode32: function(num) {
      let res = '';
      let n = Number(num);
      if (n === 0) return '0'.repeat(7);
      while (n > 0) {
        res = CROCKFORD_ALPHABET[n % 32] + res;
        n = Math.floor(n / 32);
      }
      return res.padStart(7, '0');
    },
    decode32: function(str) {
      let s = String(str).toUpperCase().replace(/O/g, '0').replace(/[IL]/g, '1');
      let num = 0;
      for (let i = 0; i < s.length; i++) {
        const val = CROCKFORD_ALPHABET.indexOf(s[i]);
        if (val === -1) continue;
        num = num * 32 + val;
      }
      return num;
